...
BEGIN
    ...
    -- Create a search pattern that matches both exact words and prefixes
    -- For a word 'test', this creates: '(test | test:*)'
    search_terms := string_agg('(' || word || ' | ' || word || ':*)', ' & ')
    FROM unnest(string_to_array(lower(search_query), ' ')) as word
    WHERE length(word) >= 2;  -- Only search for words with 2 or more characters

    -- If no valid search terms, return empty result
    IF search_terms IS NULL OR search_terms = '' THEN
        RETURN;
    END IF;
