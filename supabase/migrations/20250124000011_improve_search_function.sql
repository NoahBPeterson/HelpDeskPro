...
BEGIN
    ...
    -- Split into individual prefix terms
    search_terms := array_to_string(
        array(select lower(unnest(string_to_array(search_query, ' '))) || ':*'),
        ' & '
    );

    RETURN QUERY
    WITH ranked_matches AS (
        -- Search in tickets using simple config for exact prefix matching
        SELECT 
            t.id as ticket_id,
            t.title,
            t.description,
            ...
            t.workspace_id,
            ts_rank(
                to_tsvector('simple', t.title || ' ' || t.description),
        ...
        WHERE 
            ...
            to_tsvector('simple', t.title || ' ' || t.description) @@ to_tsquery('simple', search_terms)

        UNION ALL

        -- Search in comments using simple config
        SELECT 
            t.id,
            t.title,
            t.description,
        ...
        WHERE 
            t.workspace_id = p_workspace_id AND
            to_tsvector('simple', c.content) @@ to_tsquery('simple', search_terms) AND
            (
